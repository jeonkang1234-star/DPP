...
DEMO_COMPANIES = [
    ...
    # 2026-08-23 추가. 시연 대표 제조사(강 요청). 실존하지 않는 가상 회사이고,
    # 138-86-47212는 위 체크섬을 통과하며 시드/기존 mock 어디와도 겹치지 않는다.
    # 철강 목데이터 문서(docker/mock-documents/steel/*)의 발행사는 여전히
    # 스트럭타스틸㈜이다 - 이 계정으로 발급할 DPP의 '제조사명' 칸에는 과천제철을 넣게 되니,
    # 화면 값과 첨부 PDF의 발행사가 서로 다르게 보인다. 정합성을 맞추려면
    # mock_data.py의 ISSUERS['STEEL']도 과천제철로 바꾸고 문서를 다시 생성해야 한다.
    ("사업자등록증_과천제철.pdf", "과천제철 주식회사", "138-86-47212", "정하람",
     "제조업", "1차 철강 제조", "경기도 과천시 과천대로 132", "2011년 09월 15일"),
    # 2026-08-23 추가. 위 과천제철과 짝이 되는 시연용 협력사(강 요청). 반드시 제조사와
    # 다른 사업자등록번호여야 한다 - 같은 번호로 가입하면 그 조직(제조사)에 합류해서
    # 제조사 화면이 뜬다. 138-81-30526도 체크섬을 통과하며 위 목록/시드 어디와도 겹치지
    # 않는다. 업태·종목은 표시용일 뿐이고 실제 협력사 역할(RAW_SUPPLIER/TEST_LAB/
    # RECYCLER)은 제조사가 초대할 때 고르므로 이 문서가 역할을 제약하지는 않는다.
    ("사업자등록증_안양협력.pdf", "안양협력 주식회사", "138-81-30526", "배시우",
     "도소매업", "고철·스크랩 매입 및 원부자재 공급", "경기도 안양시 동안구 학의로 282", "2015년 04월 27일"),
]
